...
Menu = True          #Used to start the menu overlay loop
...
def game_win(): # defines a functions called game_win which prints a sequence of ascii characters to spell out the words: you win. Ths will be displayed to the user when they sink all enemy ships
    ...
    print('   ██║   ╚██████╔╝╚██████╔╝    ╚███╔███╔╝██║██║ ╚████║')
    print('   ╚═╝    ╚═════╝  ╚═════╝      ╚══╝╚══╝ ╚═╝╚═╝  ╚═══╝')
    blanks(2) # calls the blanks function to create blanks spaces in between words. This makes things easier to read
    move_on = input('Enter - continue to main menu') # makes sure the user acknowledges their win before continuing

def game_loser(): # prints the an a sequence of ascii characters to tell the user they are pathetic for losing to a bot
    blanks(5) # blanks make things easier to read
    print('██████╗  █████╗ ████████╗██╗  ██╗███████╗████████╗██╗ ██████╗')
    print('██╔══██╗██╔══██╗╚══██╔══╝██║  ██║██╔════╝╚══██╔══╝██║██╔════╝')
    print('██████╔╝███████║   ██║   ███████║█████╗     ██║   ██║██║  ')
    print('██╔═══╝ ██╔══██║   ██║   ██╔══██║██╔══╝     ██║   ██║██║')
    ...
    print('╚═╝     ╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚══════╝   ╚═╝   ╚═╝ ╚═════╝')
    blanks(5)
    move_on = input('Enter = continue to main menu') # makes sure the user has acknowledged their loss before continuing
...
def userPointSelect():   #Creates function to select a point on the grid and verifies if it is a valid coordinate
    ...
    while error_checker == True:  #Loop begins to get input and check for errors
        try:
            user_point = input('Enter a coordinate(e.g. "D2"): ')  #Asks user for coordinate value
            user_point = user_point.lower()   #Lower cases input for value check
...
def compShot():
    blanks(2) # calls blank spaces function to creates spaces between visual instances
    ...
    if ((grid[ship_row])[x_range]) == 'S': # creates an if statement that will proceed if the item within the indexed list is 'S'
        ...
        compShot() # if ther comp's shot hit, then the function is called again, so the it is the cpu's turn again.
    elif ((grid[ship_row])[x_range]) == 'X' or ((grid[ship_row])[x_range]) == 'O':
        compShot()
...
def userShot():  #Creates function to ask user to select a coordinate to place a shot on the program's grid 
    ...
    if ((cpu_grid[ship_row])[x_range]) == 'E':   #If user's point matches with an enemy ship, this will display
        ...
        if cpu_ships != []:
            userShot() # calls the function in itself if the user hit their shot. This means it is their turn again.
    elif ((visual_grid[ship_row])[x_range]) == 'X' or ((visual_grid[ship_row])[x_range]) == 'O':
        blanks(1)
        print('You have already placed a shot there')
        move_on = input('OK!')
        userShot()
        ...
        #blanks(2)   #Calls function to output a certain amount of blanks
...
def compShotsMiss(): # defines a function which will be used to tell the user that the cpu missed its shot
    print('\nThe enemy fired. It missed.') # tells the user the cpu shot missed
    moveon = input('OK!')

def blanks(number): # defines a function called blanks(n) which will create blank spaces
    print('\n' * number) # prints as many blank spaces as put in the function
...
visual_grid = [visual_row1,visual_row2,visual_row3,visual_row4,visual_row5,visual_row6,visual_row7,visual_row8,visual_row9,visual_row10]  #Creates list of visual grid rows 
...
while Menu == True: # starts a while loop that starts by printing a game welcoming menu + title
    print(' \n \n \n \n ')
    print('                                                  .  o ..')
    print('                                                  o . o o.o')
    print('                                                       ...oo')
    ...
    print('                                         |_|    ')
    print(' \n \n \n \n ')
    print('Welcome to the game of Battleship!') # welcomes the user to the game
    print(' ')
    print('S - Start the game') # gives the user options to choose at the menu
    print('H - Help / Instructions')
    print('Q - Quit the game')
    print(' ') # spaces make thinkgs easier to read
    ...
    if user_choice == 'H' or user_choice == 'h':    #If user types h for help, program displays information about the program
        ...
        Menu = False    #Ends the whole program
        Flag_user = False
    else:
        print('Invalid Command. Please enter a letter again.')   #If user types a letter that isn't s, h, or q, this will display
    ...
    while Flag_user == True: #Main loop program begins
        ...
        while user_ship_placement == True:   #Begins loop to get user's input on ship placement
            ...
            while error_checker == True:  #Loop begins to check error in ship length input
                try:
                    ship_length = input('Choose a ship length (2 - 5): ')   #Asks user for ship length value
        ...
        while cpu_ship_placement == True:  #Begins loop to add program's ships to their grid
            ...
            while ship_length < 6:   #Begins loop to add program's ships until all ships have been successfully placed
                ...
                x_range = cpu_point[0]   #Slices input for x and y coordinate values
                ship_row = cpu_point[1]
                ship_row = int(ship_row)
                x_range = int(x_range)
                
                cpu_ship_direction = random.randint(1,2) # creates a var 'cpu_ship_direction' and puts a random value between 1 and 2 in it. This will be used to determine the direction the cpu ship is facing
                if cpu_ship_direction == 1: # if the value inside the var is 1...
                    cpu_ship_direction = 'h' # changes the value inside the var to 'h'. This will allow the program to place the ship horizontally
                elif cpu_ship_direction == 2: # if the value inside the var is 2...
                    cpu_ship_direction = 'v' # changes the value inside the var to 'v'. This will allow the program to place the ship vertically

                ship_length = cpuShipPlacement(cpu_ship_direction,cpu_grid,ship_row,ship_length,x_range,cpu_ships,cpu_point)    #Calls function to place enemy ships on grid

            ship_length = 3  #Last ship to be placed set to 3
            while ship_length == 3:  #Loop begins to place last ship until successful
                cpu_point = cpuPointSelect()   #Calls function to get program point value
                x_range = cpu_point[0]  #Slices input for x and y coordinate values
                ship_row = cpu_point[1]
                ship_row = int(ship_row)
                x_range = int(x_range)

                cpu_ship_direction = random.randint(1,2)  #Program chooses number between 1 and 2 to select ship placement direction
                if cpu_ship_direction == 1:  #If program selects one, ship direction is set to horizontal
                    cpu_ship_direction = 'h'
                elif cpu_ship_direction == 2:  #If program selects one, ship direction is set to vertical
                    cpu_ship_direction = 'v'

                ship_length = cpuShipPlacement(cpu_ship_direction,cpu_grid,ship_row,ship_length,x_range,cpu_ships,cpu_point)  #Calls function to place enemy ships on grid
                
            cpu_ship5.append(cpu_ship2[3])  #Adds each point to new ship list so it can be registered as a new ship
            cpu_ship5.append(cpu_ship2[4])
            cpu_ship5.append(cpu_ship2[5])
            del(cpu_ship2[3:6])  #Deletes new ship points that were placed in the other list
            del(cpu_ships[0:2])  #Deletes none values in 2d list so it can be used it ship sinking verification
            cpu_ship_placement = False  #Ends computer ship placement loop
        
        print('\nThe CPU will shoot first')
        move_on = input('OK!')

        game_loop = True # creates a flag called game_loop and sets it to true. This will be used in the game, when the cpu and the user are going back and forth between turns.
        
        while game_loop == True: # creates a while loop using the game_loop flag from above
          
            compShot() # starts the loop with the cpu's shot turn by calling the function

            if row1.count('S') == 0 and row2.count('S') == 0 and row3.count('S') == 0 and row4.count('S') == 0 and row5.count('S') == 0 and row6.count('S') == 0 and row7.count('S') == 0 and row8.count('S') == 0 and row9.count('S') == 0 and row10.count('S') == 0: # checks the users list grid to see if there are no 'S''s, which would mean the user lost
                print('The enemy sunk all of your ships, you lost.') # undramatically tells the user they lost
                game_loser()
                game_loop = False # stops the rounds from proceeding
                Menu = True # brings the menu by making the flag true
                Flag_user = False  #stops the initial game loop (ship placement, etc.)

            else: userShot() # proceeds to the users turn with the userShot() function

            if cpu_ships[0] == [] and cpu_ships[1] == [] and cpu_ships[2] == [] and cpu_ships[3] == [] and cpu_ships[4] == []: # checks if all the lists are empty every round to know when to stop the game
                print('Congratulations! You sunk all of the enemy ships!') # prints a congrat statement if the above is true, and the user sank all enemy ships
                game_win() # calls the game_win() function to make the user happy
                game_loop = False # makes the loop false to stop the rounds from proceeding
                Menu = True # makes the flag true to bring up the menu 
                Flag_user = False # stops the inital game loop (ship placement, etc.)